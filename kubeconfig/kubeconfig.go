...
// TODO(christopherhein) Refactor this to use client-go types
const (
	kubeconfigTemplate = `---
kind: Config
apiVersion: v1
...
func GenerateKubeconfig(user, clusterName, apiserverDomain string, groups []string, rootCA *pki.CrtKeyPair) (string, error) {
	caPair, err := pki.NewClientCrtAndKey(user, rootCA, groups)
	if err != nil {
		return "", err
	}
...
func generateKubeconfigUseCertAndKey(clusterName string, ips []string, apiserverCA *x509.Certificate, caPair *pki.CrtKeyPair, username string) (string, error) {
