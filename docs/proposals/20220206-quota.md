...
* Cross namespace quota admission control needs a centralized engine to validate resource creation. Traditional approach is to use validation Webhook. However, using Webhook in multi-tenant cluster environment has following issues:
    ...
    * for a Webhook installed in super cluster: it becomes a bottle neck for all tenants and impacts multiple threading model implemented in pod syncer.  In addition the Webhook needs to map namespaces with tenants, and other information originally kept in syncer to performance validation, which is not efficient.
...
* Propose a quota validation plugin for user to integrate proprietary quota management functions into pod-syncer.
...
* Enforce tenant overall quota usage in super cluster but allow tenant freely creating namespace, and managing separate quota in virtual cluster.
* Enable cross namespace quota admission control in super cluster to reduce resource fragementation.
* Allow cloud admin to define own quota validation implementation and easy integration into pod-syncer
* Handle tenant’s resource requests in parallel and minimize interferences of large batch requests from noisy tenant.
...
* the pod-syncer first locks tenant, then get current tenant quota usage, compares available quota with requested quota from pod, if there is enough quota, pod syncer will creates the pod in super cluster and removes the per tenant lock.
* if there no enough quota, the per tenant lock will be released. The creation reconcile routine can return with no-requeue option, so the pod will not be putting back into reconcile queue immediately. The un-synced pod will be in pending state. The pod-syncer patroller will re-queue the pending pod later. 


**Validation Plugin**

A ValidationPluginInterface will be defined to present all types of validation plugins. User proprietary quota validation plugin will be derived from it.
...
Users need to register their proprietary quota validation logic in init function:
...
#### Resource Quota Object Syncing

A resource quota object can be deployed to each namespace in super cluster. The resource qutoa object is used for counting resource usage for each priority class. The resource quota object is fully owned by syncer and does not need to be synced upward to virtual cluster.  

If tenant wants to define resource quota object in tenant virtual cluster, the resource quota object only interacts with tenant’s virtual cluster api-server. It will not be synced downward to super cluster. There is not dependency between the quota admission control in virtual cluster and in super cluster.
